...
export function AIScheduleProvider({ children }) {
  ...
  /** 주간 배분 생성 — 오직 사용자 명시적 호출로만 실행 */
  const generateSchedule = useCallback(async (homeworks, schedules, googleEvents, weekMonday, slotSettings) => {
    // 중복 호출 원천 차단 (동기 ref)
    ...
    try {
      const response = await fetch('/api/schedule-homework', {
        ...
        body: JSON.stringify({ homeworks, schedules, googleEvents, weekStart, slotSettings }),
      })
